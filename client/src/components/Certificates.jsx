...
export default function Certificates({ certificates = [] }) {
  ...
  return (
    <section id="certificates" className="min-h-[70vh] flex py-6 flex-col items-center relative overflow-hidden">
        ...
        >
          {certificates.map((cert, i) => (
            <motion.article
              ...
              whileHover={{ y: -10 }}
              className="group bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl p-6 hover:border-white/20 transition duration-300"
            >
              ...
              {cert.img && (
                <div className="mb-4 overflow-hidden rounded-xl">
                  <img src={cert.img} alt={cert.title} className="w-full h-40 object-cover transition-transform duration-300 group-hover:scale-105" />
                </div>
              )}
              <button
